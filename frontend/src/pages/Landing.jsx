...
function RoleSection({
  ...
  return (
    ...
    >
      <div
        className={`mx-auto flex w-full max-w-5xl flex-col items-center gap-12 md:flex-row ${reverse ? "md:flex-row-reverse" : ""}`}
      >
        <div
          className={`reveal ${inView ? "reveal-visible" : ""} flex-1 text-center md:text-left`}
        >
          <Icon
            size={64}
            strokeWidth={1.5}
        ...
        >
          ...
          <Button
            onClick={() =>
              navigate(`/register?role=${encodeURIComponent(title)}`)
            }
...
export default function Landing() {
  ...
  return (
    <div
      className="h-screen overflow-y-auto bg-coal-950"
    ...
    >
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-display text-xl font-semibold tracking-wide text-smoke-100">
            TRANSITOPS
          </span>
        </div>
        ...
        <p className="max-w-xl text-smoke-400">
          One dashboard per role. Every business rule enforced automatically —
          not by trusting someone to remember.
        </p>
